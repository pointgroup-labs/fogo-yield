...
export const configSchema = z.object({
  ...
  CHECKPOINT_PATH: z.string().default('./cranker-checkpoint.json').describe('On-disk checkpoint file (per-emitter watermarks). Empty string disables persistence (in-memory watermarks still apply per process). On-chain idempotency makes a lost checkpoint a one-time backfill, never a missed dispatch. The Docker image overrides this to /app/state/cranker-checkpoint.json, persisted across redeploys via the cranker-state volume.'),
